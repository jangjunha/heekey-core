enum TypeA자음PreParser<T: Equatable, U: Equatable> {
  /// 자음을 초성 혹은 종성으로 분류

  typealias Input = TypeA자음PreParserInputToken<T, U>
  typealias Output = TypeA자음PreParserOutputToken<T, U>
  ...
  static func parse_next(last: Output?, p1: Input, p2: Input?) -> Output {
    ...
    case let (_, .other(p1), _): return .other(p1)
    ...
    case let (.종성(.ㄴ), .자음(p1), .자음(_)) where [.ㅈ, .ㅎ].contains(p1),
      let (.종성(.ㄴ), .자음(p1), .other(_)) where [.ㅈ, .ㅎ].contains(p1),
      let (.종성(.ㄴ), .자음(p1), nil) where [.ㅈ, .ㅎ].contains(p1):
      return .종성(p1)
    case let (.종성(.ㄹ), .자음(p1), .자음(_)) where [.ㄱ, .ㅁ, .ㅂ, .ㅅ, .ㅌ, .ㅍ, .ㅎ].contains(p1),
      let (.종성(.ㄹ), .자음(p1), .other(_)) where [.ㄱ, .ㅁ, .ㅂ, .ㅅ, .ㅌ, .ㅍ, .ㅎ].contains(p1),
